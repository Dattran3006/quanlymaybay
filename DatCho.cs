    ...
    public partial class DatCho : Form
    {
        ...
        private void cb_1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable giave = cn.ExecuteData("SELECT * FROM DONGIA WHERE  MATUYENBAY='" + matuyen + "' AND MAHANGVE='" + cb_2.SelectedValue + "'");
        ...
        {
            string sql = "INSERT INTO PHIEUDATCHO(MAKHACHHANG,CMND,MACHUYENBAY,MAHANGVE,SOGHEDAT,NGAYDAT,GiaTien) VALUES (N'" + MAKHACHHANG+"','"+ CMND + "',N'" + MACHUYENBAY + "',N'" + MAHANGVE + "'," + SOGHEDAT + ",N'" + NGAYDAT.ToString("hh:mm:ss  MM/dd/ yyyy") + "'," + GiaTien + ")";
        ...
        {
            string sql = "UPDATE PHIEUDATCHO SET [MAKHACHHANG]="+ MAKHACHHANG+",[CMND]=" + CMND + ",[MACHUYENBAY]=" + MACHUYENBAY + ",[MAHANGVE]=" + MAHANGVE + ",[SOGHEDAT]=N'" + SOGHEDAT + "',[NGAYDAT]=" + NGAYDAT.ToString("hh:mm:ss  MM/dd/ yyyy") + ",[GiaTien]=N'" + GiaTien + "' WHERE [MAPHIEUDATCHO]=" + MAPHIEUDATCHO + "";
            return cn.ExecuteNonQuery(sql);
        }
        ...
        private void txt_5_TextChanged(object sender, EventArgs e)
        {
            try
            {
                string sql = "Select * from KHACHHANG where MAKHACHHANG ='" + txt_5.Text + "'";
                DataTable dt = cn.ExecuteData(sql);
                if (dt.Rows.Count > 0)
                {
        ...
        {
            string sql = "select GIA from DONGIA where MAHANGVE='" + cb_2.SelectedValue.ToString() + "' and MACHUYENBAY='" + cb_1.SelectedValue.ToString() + "'";
            DataTable dt = cn.ExecuteData(sql);
            if (dt.Rows.Count > 0)
            {
