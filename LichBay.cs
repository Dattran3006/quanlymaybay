...
namespace QuanLyMayBay
{
    public partial class LichBay : Form
    {
        ...
        private void bt_them_Click(object sender, EventArgs e)
        {
            try
            {
                if (INSERT(textBox1.Text, dateTimePicker1.Value, int.Parse(textBox6.Text), int.Parse(textBox2.Text), int.Parse(textBox3.Text), comboBox1.SelectedValue + "-" + comboBox2.SelectedValue, comboBox3.SelectedValue.ToString()) != -1)
                {
                    dataGridView1.DataSource = SelectAll();
